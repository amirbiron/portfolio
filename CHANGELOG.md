## [2026-08-19] עדכון הנחיות סגנון לסוכן ה-AI של CodeKeeper + קישור לאתר התיעוד

**קבצים שהשתנו:**
- `server/index.ts` — (שונה) שכתוב של `SYSTEM_PROMPT` בנקודת הקצה `/api/ask-ai`

**פירוט:**
ה-System Prompt של סוכן ה-AI שעונה על שאלות לגבי CodeKeeper אורגן מחדש לסעיפים (סגנון הניסוח / תוכן התשובה / קישור לאתר התיעוד / כללי), ונוספו לו הנחיות סגנון חדשות:

1. **סגנון הניסוח** (סעיף חדש): לענות ישר לשאלה בלי הקדמות ובלי חזרה על השאלה; לדבר בגובה העיניים בשפה פשוטה, בלי שפה גבוהה ובלי טון מתנשא; לא להפריז באיכות הפיצ'רים ולהימנע מסופרלטיבים כמו "מהפכני" או "עוצמתי במיוחד"; לספק מידע טכני אמין ואמיתי לפי מסמך התכונות בלבד, ואם משהו לא מופיע בו — לומר שלא יודע במקום להמציא.

2. **קישור לאתר התיעוד** (סעיף חדש): הסוכן רשאי לצרף את הקישור `https://amirbiron.github.io/CodeBot/` רק אחרי שהמשתמש שאל בערך 3 שאלות בשיחה (לא בשאלה הראשונה), ופעם אחת בלבד לכל שיחה — עם הנחיה מפורשת לעבור על היסטוריית השיחה ולא לצרף שוב אם הקישור כבר הופיע בתשובה קודמת. הבדיקה אפשרית כי הפרונט שולח את היסטוריית השיחה בכל בקשה ל-`/api/ask-ai` (עד 20 הודעות אחרונות).

3. **התאמות בניסוח הקיים**: שם המוצר בפתיח שונה מ-CodeBot ל-CodeKeeper (עם הבהרה שבמסמך התיעוד הוא מופיע גם בשם CodeBot ושמדובר באותו מוצר) כדי להתאים לשם שמופיע בממשק; ההנחיה על "החשיבה השורשית" נוסחה מחדש בשפה פשוטה יותר; הטון הוגדר "מקצועי, ענייני וצנוע" במקום "צנוע אבל בטוח בעצמו", כדי לא לסתור את ההנחיה החדשה נגד הפרזה.

ההנחיות הקיימות שנשמרו: לא להעתיק רשימות ארוכות (3-4 פיצ'רים מרכזיים + הצעה לשאול על נושא ספציפי), התאמת עומק התשובה לקהל (טכני מול עסקי), מענה בעברית אלא אם השאלה באנגלית, ותשובות של עד 3-4 פסקאות עם הצעה לשאלת המשך.
